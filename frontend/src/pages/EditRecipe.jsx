...
const EditRecipe = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToastId = toast.loading("Saving recipe...")

    try {
      await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/api/recipes/${recipeData._id}`,
        updatedRecipeData,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      toast.success("Recipe saved successfully.", { id: loadingToastId })
      navigate(`/recipe/${recipeData._id}`)
    } catch (error) {
      toast.error("Failed to upload recipe.", { id: loadingToastId })
    }
  }
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-avenir-regular text-gray-600">
              Make changes to your existing recipe.
            </span>
          </div>
        </div>
        <div>
